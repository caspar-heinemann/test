...
$skip_csrf_check = apply_filters( 'vfb_skip_csrf_check', false, $form_id );
// Allow CSRF check to be skipped
if ( !$skip_csrf_check ) :
	$csrf       = isset( $_POST['_vfb-csrf-token'] ) ? esc_html( $_POST['_vfb-csrf-token'] ) : false;
	$csrf_token = $this->session->get( 'vfb-token-' . $form_id );

	// Test if CSRF token has been set
	if ( !$csrf ) {
		wp_die( apply_filters( 'vfb_str_security_csrf_set', __( 'Security check: the security token does not appear to be set' , 'visual-form-builder-pro' ) ), '', array( 'back_link' => true ) );
	}
	else {
		// Test if the CSRF token matches the session variable; fail silently
		if ( $csrf !== $csrf_token['vfb-csrf-token'] )
			return;
	}
endif;

// Reset the session
$this->session->set( 'vfb-token-' . $form_id, null );
...
if ( !has_action( 'vfb_override_email_' . $form_id ) ) :
	...
	$skip_email = apply_filters( 'vfb_send_email', $settings_send_email, $form_id );
...
if ( !empty( $form_settings->form_notification_setting ) ) :
	...
	if ( !has_action( 'vfb_override_email_notify_' . $form_id ) ) :
		...
		$skip_email_notify = apply_filters( 'vfb_send_notify_email', $settings_send_email_notify, $form_id );
